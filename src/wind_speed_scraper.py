"""
Wind Speed Data Scraper with Hourly Aggregation
================================================
Scrapes per-minute wind speed data and aggregates to hourly averages
"""
...
API_ENDPOINT = "https://api-open.data.gov.sg/v2/real-time/api/wind-speed"
START_DATE = "2025-01-01"
END_DATE = "2025-11-01"

# Directory configuration
DATA_DIR = "data"
CHECKPOINT_DIR = os.path.join(DATA_DIR, "checkpoints", "wind-speed")
OUTPUT_DIR = os.path.join(DATA_DIR, "wind-speed")
...
def scrape_date(api_endpoint, date_str):
    """
    Scrape wind speed data for a specific date with pagination handling
...
def scrape_wind_speed_data(api_endpoint, start_date, end_date, save_frequency=10, resume=True):
    """
    Scrape wind speed data by iterating through dates with checkpoint system
    ...
    print(f"\nScraping wind speed data from {start_date} to {end_date}")
...
def aggregate_to_hourly(readings, stations_metadata, output_dir=OUTPUT_DIR):
    """
    Process minute-level readings and aggregate to hourly averages per station
    ...
    print(f"  After removing nulls: {len(df)} data points")

    # Create hour column for aggregation
    df['hour'] = df['timestamp'].dt.floor('H')

    # Aggregate to hourly averages by station
    print(f"\nAggregating to hourly averages...")
    hourly_df = df.groupby(['station_id', 'hour']).agg({
        'wind_speed': 'mean',
        'timestamp': 'count'  # Count of minute readings per hour
    }).reset_index()
    ...
    for station_id in sorted(hourly_df['station_id'].unique()):
        ...
        station_df = station_df[['timestamp', 'station_id', 'station_name', 
                                 'wind_speed_avg', 'reading_count', 
                                 'latitude', 'longitude']]

        # Save to CSV
        filename = os.path.join(output_dir, f"{station_id}-wind-speed-hourly-230701-251101.csv")
...
if __name__ == "__main__":
    print("=" * 70)
    print("Wind Speed Data Scraper with Hourly Aggregation")
    print("=" * 70)
    print(f"\nConfiguration:")
    print(f"  Data directory: {os.path.abspath(DATA_DIR)}")
    print(f"  Checkpoints: {os.path.abspath(CHECKPOINT_DIR)}")
    print(f"  Output CSVs: {os.path.abspath(OUTPUT_DIR)}")
    print(f"\nData will be aggregated from per-minute to hourly averages")
    ...
    try:
        ...
        readings, stations = scrape_wind_speed_data(API_ENDPOINT, START_DATE, END_DATE, 
                                                      save_frequency=SAVE_FREQUENCY, resume=True)

        # Process and aggregate to hourly
        print("\n[2/2] Aggregating to hourly averages and saving by station...")
